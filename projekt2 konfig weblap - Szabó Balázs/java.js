...
function populate() { //Nagyon hasznos: egy adott HTML elementnek bármilyen néven lehet megadni attribute-okat.
    ...
    switch (reszek[i].type) {
      ...
      case "type1":
        document.getElementById(
          "mb"
        ).innerHTML += `<option value="${reszek[i].price}" color="${reszek[i].image}" detail="${reszek[i].detail}">${reszek[i].name}</option>`;
        break;
      case "CPU":
      case "type2":
        document.getElementById(
...
function generate() {
  ...
  for (let i = 0; i < 200; i++) {
    ...
    n = t + getRndInteger(100, 999);
    d = t+" Detail" + getRndInteger(100, 999);
    p = getRndInteger(10000, 100000);
    c = "images/" + colors[getRndInteger(0, 7)] + ".png";
...
function teljesAr(actual) {
  ...
  try { //Minden látható kép teljesen másik fájl (Ezt is szerettem volna bemutatni, nem a szinek váltását). 
    //A kiirt szöveg a kép alatt lehetne bármilyen hosszú, akkor is működne.
    if (actual.value > 0){
      var select = document.getElementById(actual.id);
      var selectedOption = select.options[select.selectedIndex];
      var color = selectedOption.getAttribute("color");
      var detail = selectedOption.getAttribute("detail")
      document.getElementById(
        "image"
      ).innerHTML = `<img src="${color}"></img><p>${detail}</p>`;
    }
  } catch (error) { //Volt egy hibakód, de hiba nem volt. Emiatt hibakód sincs.
    console.error();
  }
}
...
var korertVis = 0;
function ertekel() {
  n = parseInt(document.getElementById("rate").value);
  switch (n) {
    case n > 5: {
      temp = {
        num: 5,
  ...
  korertbodyCode += `<hr style="color:white;"><p>${temp.num}/5</p><p>${temp.text}</p>`;
  korertCode =
    `<div class="col-sm-12 col-md-6" id="korertCol">
  <div class="inside bottom right" id="korert"><input type="button" value="Korábbi értékelések eltüntetése"
      onclick="korjeldel()">` +
    korertbodyCode +
    `</div>
</div>`;
  if (korertVis == 1) {
    document
      .getElementById("korertCol")
      .parentNode.removeChild(document.getElementById("korertCol"));
    document.getElementById("bottomrow").innerHTML += korertCode;
  }
}
...
//Ha a korábbi értékelések az mindig ott van, csak láthatatlanul, 
//akkor ha a div-ek egymás alatt vannak, mert kicsi a képernyő, és ha korábbi értékelések div láthatatlan,
//akkor rengeteg üres hely lesz a weblap alján. Ezzel a megoldással nincs üres hely,
//mert a korábbi értékelések div vagy látható, vagy nincs ott.
...
function readFile() { //Forrás: ChatGPT
  document.getElementById("pop").style.visibility = "hidden";
